...
- `heatmap-analyzer` (UX): Mapa de atención visual **100% client-side**. Motor en `lib/heatmap-engine.ts` (Canvas 2D + filtro `blur` nativo) que combina cuatro señales — bordes (Sobel), contraste de luminancia, saturación, sesgo central F-Z. Las señales se precalculan al cargar la imagen; los sliders re-renderizan en vivo con ~60ms de debounce.
  - **Sección "Análisis"**: cantidad de puntos (umbral), tamaño del foco (Blur Spread `5–50px`, default 20), intensidad, toggle "Activar sesgo central" + slider de peso central (default `0.15`), pesos de bordes / contraste / color.
  - **Sección "Detección de picos"**: toggle "Mostrar picos" (overlay numerado), toggle "Mostrar intensidades" (% bajo cada marcador), distancia mínima entre picos (`peakRadius` `10–50px`, default 20), umbral de intensidad (`peakThreshold` `50–200`, default 100), máximo de picos (`maxPeaks` `2–10`, default 5).
  - **Detección**: `detectPeaks()` aplica Non-Maximum Suppression sobre el mapa post-blur. El radio NMS, el umbral y el tope de picos vienen ahora directamente de `HeatmapOptions` (antes se derivaban de spread/threshold).
  - **Render**: los marcadores (círculo rojo→naranja por rank, número y pill de intensidad) van solo al **preview**; el export `renderFullBlob` queda limpio. Los picos se exponen en `engine.lastPeaks` después de cada render.
  - `privacy: 'local'`, **no requiere microservicio**.

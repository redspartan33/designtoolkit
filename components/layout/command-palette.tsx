...
export function CommandPalette() {
  const { commandPaletteOpen, setCommandPaletteOpen, toolUsageCounts } =
    useUIStore();
  ...
  const filtered: Tool[] = query.trim()
    ? sortedTools.filter((tool) => {
        ...
        const tName = t(`tools.${tool.id}.name`, tool.name).toLowerCase();
        const tDesc = t(
  ...
  const showingRecents =
    !query.trim() && Object.keys(toolUsageCounts).length > 0;
      ...
      <div className="glass w-full max-w-lg mx-4 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in-0 zoom-in-95 duration-150">
        ...
        <div className="p-2 max-h-[360px] overflow-auto">
          ...
          <div className="px-2 py-1.5 text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 flex items-center gap-1.5 mb-1">
            {showingRecents ? (
              <>
                <Clock className="h-3 w-3" /> {t("palette.recent")}
              </>
            ) : showingAll ? (
              <>
                <Hash className="h-3 w-3" /> {t("palette.tools")}
              </>
            ) : (
              <>
                <Search className="h-3 w-3" /> {t("palette.results")}
              </>
            )}
          </div>
        ...
        <div className="px-4 py-2.5 border-t border-white/10 flex items-center gap-3 text-[10px] text-muted-foreground/60">
          <span>
            <kbd className="font-mono bg-muted/60 px-1 py-0.5 rounded">↑↓</kbd>{" "}
            {t("palette.navigate")}
          </span>
          <span>
            <kbd className="font-mono bg-muted/60 px-1 py-0.5 rounded">↵</kbd>{" "}
          ...
          <span>
            <kbd className="font-mono bg-muted/60 px-1 py-0.5 rounded">Esc</kbd>{" "}
            {t("palette.close")}
          </span>
        </div>
      </div>
